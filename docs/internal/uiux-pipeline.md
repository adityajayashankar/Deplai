...
3. A bounded planner, tool-driven editor, and independent read-only reviewer share request, token, and time limits. Product inference uses Connector's authenticated `/api/ai/chat` gateway and its platform `z-ai/glm-5.3-flash` route, verified catalog pricing, quota scheduling, and wallet reservation. Missing gateway configuration blocks a product run instead of switching to a worker provider key. No alternate model or user key is accepted. GLM is paid platform inference. CSS/SCSS-only edits treat process-only reviewer complaints as advisory after local guards; invalid review contracts and concrete conflicts remain blocked. This is a custom bounded graph, not an installation of the Deep Agents SDK.
...
- Provider capacity, quality, and quotas change. Rate-limit cooldowns and retries are bounded; they cannot remove shared account quotas. Other platform agents sharing the key also consume quota.
...
UI/UX GLM migration: worker calls are gateway-only, with no standalone provider-key fallback. Editor output defaults to 4,000 tokens; source history compacts and editor calls reserve room for independent review. Observed successful-call provider cost is retained in run usage and shown in the workspace; it is not an account invoice or a hard dollar cap. Builds and live preview remain omitted.
