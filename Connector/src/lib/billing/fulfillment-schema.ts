import { query } from '@/lib/db';

function isIgnorableAlterError(error: unknown): boolean {
  const code = (error as { code?: string }).code;
  return (
    code === 'ER_DUP_FIELDNAME'
    || code === 'ER_DUP_KEYNAME'
    || code === 'ER_NO_SUCH_TABLE'
    || code === 'ER_BAD_TABLE_ERROR'
  );
}

function isIgnorableCreateError(error: unknown): boolean {
  const code = (error as { code?: string }).code;
  return code === 'ER_TABLE_EXISTS_ERROR' || code === 'ER_DUP_KEYNAME';
}

async function runSchema(sql: string, ignorable: (error: unknown) => boolean): Promise<void> {
  try {
    await query(sql);
  } catch (error) {
    if (ignorable(error)) return;
    throw error;
  }
}

let ready: Promise<void> | null = null;

export async function ensureFulfillmentSchema(): Promise<void> {
  if (!ready) {
    ready = (async () => {
      await runSchema(
        `CREATE TABLE IF NOT EXISTS billing_webhook_events (
          id VARCHAR(191) PRIMARY KEY,
          event_type VARCHAR(64) NOT NULL,
          status VARCHAR(16) NOT NULL DEFAULT 'completed',
          processed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
          updated_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
        )`,
        isIgnorableCreateError,
      );
      await runSchema(
        `ALTER TABLE billing_webhook_events
         ADD COLUMN status VARCHAR(16) NOT NULL DEFAULT 'completed'`,
        isIgnorableAlterError,
      );
      await runSchema(
        `ALTER TABLE billing_webhook_events
         ADD COLUMN updated_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP`,
        isIgnorableAlterError,
      );
      await runSchema(
        `CREATE TABLE IF NOT EXISTS billing_fulfillment_attempts (
          id VARCHAR(36) PRIMARY KEY,
          razorpay_payment_id VARCHAR(64) NULL,
          razorpay_order_id VARCHAR(64) NULL,
          source VARCHAR(32) NOT NULL,
          status VARCHAR(16) NOT NULL,
          error_message TEXT NULL,
          raw_payload JSON NULL,
          attempted_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
          completed_at TIMESTAMP NULL,
          INDEX idx_fulfillment_payment (razorpay_payment_id, attempted_at),
          INDEX idx_fulfillment_status (status, attempted_at)
        )`,
        isIgnorableCreateError,
      );
      await runSchema(
        `CREATE TABLE IF NOT EXISTS admin_audit_log (
          id VARCHAR(36) PRIMARY KEY,
          actor VARCHAR(255) NOT NULL,
          action VARCHAR(64) NOT NULL,
          target VARCHAR(191) NOT NULL,
          before_json JSON NULL,
          after_json JSON NULL,
          reason TEXT NULL,
          created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
          INDEX idx_admin_audit_created (created_at),
          INDEX idx_admin_audit_target (target, created_at)
        )`,
        isIgnorableCreateError,
      );
    })().catch((error) => {
      ready = null;
      throw error;
    });
  }
  await ready;
}
