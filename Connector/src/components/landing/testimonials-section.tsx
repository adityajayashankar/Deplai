...
const testimonials = [
  {
    quote: "Start with your source: DeplAI connects GitHub repositories and local files to security analysis and delivery planning.",
    author: "Repository context",
    role: "Workflow stage",
    company: "GitHub",
    metric: { value: "01", label: "Connect repository" },
  },
  {
    quote: "Review security findings and proposed fixes before approved changes are handed off to a pull request.",
    author: "Security remediation",
    role: "Workflow stage",
    company: "Review",
    metric: { value: "02", label: "Review remediation" },
  },
  {
    quote: "Turn guided architecture answers into a canonical deployment profile and an editable Terraform bundle.",
    author: "Infrastructure generation",
    role: "Workflow stage",
    company: "Terraform",
    metric: { value: "03", label: "Generate infrastructure" },
  },
  {
    quote: "Apply approved infrastructure, verify endpoints, and manage DeplAI-tagged resources in the AWS runtime workflow.",
    author: "Runtime operations",
    role: "Workflow stage",
    company: "AWS",
    metric: { value: "04", label: "Deploy and operate" },
  },
];

const asciiBackground = Array.from({ length: 60 }, (_, row) =>
  Array.from({ length: 100 }, (_, column) => ((row * 31 + column * 17) % 11 === 0 ? '"' : ' ')).join("")
).join("\n");
        ...
        <div className="flex items-center justify-between mb-20">
          <div>
            ...
            <h2 className={`text-4xl lg:text-5xl font-display transition-all duration-1000 ${
              ...
              From repository
              <span className="text-background/40"> to runtime.</span>
            </h2>
          </div>
              ...
              {testimonials.map((_, idx) => (
                ...
                >
                  <div 
                    ...
                    style={idx === activeIndex ? { animation: "landing-progress 8s linear forwards" } : {}}
                  />
                </button>
              ))}
            </div>
