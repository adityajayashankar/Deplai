...
const ICON_PATHS: Record<'github' | 'local', { viewBox: string; d: string }> = {
  github: {
    viewBox: '0 0 24 24',
...
export default function ProjectIcon({ type, size = 80, className = '' }: ProjectIconProps) {
  const icon = ICON_PATHS[type];
  return (
    <svg width={size} height={size} viewBox={icon.viewBox} fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      <path d={icon.d} fill="currentColor" />
    </svg>
  );
}

export function ProjectSourceThumb({
  type,
  owner,
  className,
}: {
  ...
  return (
    <div
      className={cn(
        'flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden border border-white/[0.08] bg-white/[0.03]',
        className,
      )}
      title={type === 'github' ? (owner ? owner : 'GitHub') : 'Local'}
    >
      {avatarUrl ? (
        ...
        <ProjectIcon type={type} size={18} className="text-zinc-200" />
      )}
    </div>
  );
}
