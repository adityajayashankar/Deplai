...
function LoginContent() {
  ...
  const handleUnavailableProvider = React.useCallback((message: string) => {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await submitEmailAuth("login", { email, password, remember })

    if (!result.ok) {
      setErrors({ form: result.error ?? "Unable to sign in. Please try again." })
      setIsLoading(false)
      focusErrorSummary()
      return
    }

    window.location.assign(POST_LOGIN_HREF)
  }
