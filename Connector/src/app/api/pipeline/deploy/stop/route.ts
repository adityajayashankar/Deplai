...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${AGENTIC_URL}/api/terraform/apply/stop`, {
      method: 'POST',
    ...
    if (!res.ok || data.success !== true) {
      return NextResponse.json(
        { error: data.error || data.message || 'Failed to stop deployment process.' },
        { status: res.ok ? 500 : res.status },
      );
    }

    return NextResponse.json({ success: true, message: data.message || 'Deployment stop requested.' });
  } catch (err) {
    const msg = err instanceof Error ? err.message : 'Failed to stop deployment process';
