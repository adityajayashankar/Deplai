...
Neo4j, Qdrant, and a Caddy TLS proxy. The standalone design
...
and 200 GB SSD. Allow inbound TCP ports 22, 80, and 443 only; restrict SSH to a
VPN, bastion, or known administrator IP addresses.

Point the DNS A record for `APP_DOMAIN` to the EC2 public IPv4 address before
the first start. Caddy automatically obtains and renews TLS certificates, so
...
the same HTTPS domain. These `NEXT_PUBLIC_*` values are compiled into the
...
Only Caddy publishes host ports 80 and 443. MySQL, Neo4j, Qdrant, the customization
